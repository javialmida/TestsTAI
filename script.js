...
app.startRepasoMultiTema = async () => {
    try {
        ...
        // ---------------------------------------------------------
        // Como 'preguntasFinales' tiene 'test_id', buscamos en 'state.testsCache' 
        // el nombre correspondiente y se lo pegamos al objeto.
        const preguntasConInfo = preguntasFinales.map(p => {
            const testOrigen = state.testsCache.find(t => t.id === p.test_id);
            return {
                ...p, // Copiamos todas las propiedades originales (pregunta, opciones, numero_orden...)
                nombre_test: testOrigen ? testOrigen.nombre : 'Test Varios' // Añadimos el nombre
            };
        });

        // 5. Iniciar Test
        app.resetState();
        state.q = preguntasConInfo; // <--- USAMOS EL ARRAY ENRIQUECIDO
        state.currentTestName = `📚 REPASO POR TEMA/S (${preguntasConInfo.length} PREGUNTAS)`;
        state.mode = document.querySelector('input[name="modo"]:checked').value;
