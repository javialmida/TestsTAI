    ...
    prepararRepaso: async (tipo) => {
        try {
            ...
            if (tipo === 'simulacro') {
                ...
                // 3. Traemos ID y TEST_ID para poder clasificar
                const { data: rawData } = await sb.from('preguntas')
                    .select('id, test_id')
                    .in('test_id', testsValidos);

                if (!rawData || rawData.length === 0) return alert("Error: Los tests seleccionados no tienen preguntas.");

                // --- ALGORITMO DE REPARTO EQUITATIVO (ROUND ROBIN) ---
                
                // A. Agrupamos las preguntas por tema
                const bolsasPorTema = {};
                rawData.forEach(p => {
                ...
                // C. Vamos cogiendo una de cada tema por turnos hasta llegar a 100
                ids = [];
                const keys = Object.keys(bolsasPorTema);
                let buscando = true;

                while (ids.length < 100 && buscando) {
                    buscando = false; // Asumimos que no hay más hasta encontrar una
                    for (const key of keys) {
                        if (ids.length >= 100) break; // ¡Ya tenemos 100!
                        
                        if (bolsasPorTema[key].length > 0) {
                            ids.push(bolsasPorTema[key].pop()); // Sacamos una de este tema
                            buscando = true; // Hemos encontrado al menos una, seguimos otra ronda
                        }
                    }
                }
                
                state.currentTestName = "🤖 SIMULACRO EQUITATIVO";
