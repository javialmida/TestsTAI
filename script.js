...
const app = {
    ...
    // MODIFICADO: Layout arreglado (ancho controlado) y muestra el Tiempo Total
    // MODIFICADO: Sintaxis corregida (usando dos puntos en vez de igual)
    finalizar: async () => {
        app.stopTimer(); // Paramos el reloj
    ...
    // MODIFICADO: Sintaxis corregida (usando dos puntos en vez de igual)
    confirmarSalida: () => { 
        app.stopTimer(); // Pausamos visualmente
        if(confirm("¿Deseas salir al menú principal?")) {
            location.reload(); 
        } else {
            // Si cancela, reanudamos el timer
            app.startTimer(); 
        }
    },
...
// 1. Sobrescribimos la función de abrir modal (CORREGIDA)
// 1. Sobrescribimos la función de abrir modal (CORREGIDA V2)
app.abrirModalRepasoTema = () => {
    ...
    bloquesOrdenados.forEach(bloque => {
        ...
        const detalles = document.createElement('details');
        detalles.className = 'bloque-container'; // <--- AQUÍ ESTABA EL ERROR (antes ponía details.className)
        
        // NO abrimos ningún detalle por defecto (detalles.open = false implícito)

        // Preparamos el HTML interno
        const itemsHtml = itemsParaMostrar.map(item => {
            const safeValor = item.valor.replace(/'/g, "\\'");
            
            // Si es Test -> preparaRepasoPorTestId (lee filtros + cierra modal)
            // Si es Tema -> preparaRepasoPorNombreTema (lee filtros + cierra modal)
...
app.startRepasoMultiTema = async () => {
    try {
        ...
        // ---------------------------------------------------------
        // 4.5 [NUEVO] ENRIQUECER PREGUNTAS CON EL NOMBRE DEL TEST
        // ---------------------------------------------------------
        ...
        // --- MODIFICADO: Lógica de nombre para mostrar "REPASO FALLOS" explícitamente ---
        const icono = modo === 'fallos' ? '⚠️' : '📚';
        const textoModo = modo === 'fallos' ? ' REPASO FALLOS:' : ''; // NUEVO: Texto explícito
        
        // Si hay un solo tema, lo ponemos, si hay varios, ponemos "MULTI-TEMA"
