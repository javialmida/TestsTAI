...
const app = {
                    ...
                    <div class="bloque-content">
                        ${bloques[bId].map(t => `
                            <div class="test-row" onclick="app.start(${t.id})">
                                ...
                                <small style="color:#888; margin-right:5px;">${t.temas?.nombre || ''}:</small>
                                <strong>${t.nombre}</strong>
                            </div>
                        `).join('')}
                    </div>
    ...
    registrarError: async (preguntaId, testId) => {
        try {
            const { data: ex } = await sb.from('errores').select('id, veces_fallada').eq('pregunta_id', preguntaId).single();
            if (ex) {
                await sb.from('errores').update({ veces_fallada: ex.veces_fallada + 1, ultimo_fallo: new Date().toISOString() }).eq('id', ex.id);
            } else {
                await sb.from('errores').insert([{ pregunta_id: preguntaId, test_id: testId }]);
            }
    ...
    siguiente: () => {
        if (state.cur < state.q.length - 1) { state.cur++; app.render(); }
        else app.finalizar();
    },
    ...
    finalizar: async () => {
        ...
        document.getElementById('final-stats').innerHTML = `
            <div class="resumen-final">
                <p style="font-size: 1.8em; margin-bottom: 10px;">Dominio: <strong>${((aciertos/total)*100).toFixed(1)}%</strong></p>
                <div style="display: flex; gap: 15px; justify-content: center; font-weight: bold;">
