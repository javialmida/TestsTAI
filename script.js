    ...
    prepararRepaso: async (tipo) => {
        try {
            ...
            if (tipo === 'simulacro') {
                ...
                // 2. Filtramos usando la caché (ahora segura)
                // Aseguramos que sea 'test_por_tema' para no mezclar con exámenes oficiales
                ...
                // 3. Traemos SOLO los IDs de las preguntas
                const { data: preguntasIds } = await sb.from('preguntas')
                    .select('id')
                    .in('test_id', testsValidos);

                if (!preguntasIds || preguntasIds.length === 0) return alert("Error: Los tests seleccionados no tienen preguntas.");

                // 4. Mezclamos y cogemos 100
                let poolIds = preguntasIds.map(p => p.id);
                poolIds.sort(() => Math.random() - 0.5); 
                ids = poolIds.slice(0, 100);             

                state.currentTestName = "🤖 SIMULACRO GENERADO";
